/**
 * Wire Lớp 6 question payloads to their on-disk vocab clipart.
 *
 * For every Lớp 6 question, derive the conventional image path from the
 ...
 *
 * Usage:
 *   node scripts/run-with-env.mjs local -- npx tsx scripts/wire-lop6-vocab-images.ts            # dry run
 *   node scripts/run-with-env.mjs local -- npx tsx scripts/wire-lop6-vocab-images.ts --apply    # write
 */
...
import { lop6VocabImagePath } from '../lib/lop6VocabImages';
...
function isLop6(levelName: string | null): boolean {
  return /Lớp\s*6/i.test(levelName ?? '');
}
...
async function main() {
  ...
  for (const q of questions) {
    ...
    if (!isLop6(c?.levelName ?? null)) continue;
    ...
    if (WORD_GAMES.has(q.game)) {
      ...
      if (word) {
        const p = lop6VocabImagePath(unit, word);
  ...
  console.log(`=== Wire Lớp 6 vocab images (${APPLY ? 'APPLY' : 'DRY RUN'}) ===`);
