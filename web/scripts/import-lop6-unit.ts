/**
 * Import Global Success 6 unit content (quiz/grammar/scramble/pronunciation/read_and_complete + speaking).
 *
 * Content files: scripts/data/lop6-unit{N}-content.json
 *
 * Usage (from web/):
 *   node scripts/run-with-env.mjs neon -- npx tsx scripts/import-lop6-unit.ts --unit 2
 *   node scripts/run-with-env.mjs neon -- npx tsx scripts/import-lop6-unit.ts --all
 *   node scripts/run-with-env.mjs neon -- npx tsx scripts/import-lop6-unit.ts --unit 1 --dry-run
 */
...
function parseUnitArgs(): number[] {
  if (process.argv.includes('--all')) {
    return Array.from({ length: LOP6_UNIT_COUNT }, (_, i) => i + 1);
  }
  const idx = process.argv.indexOf('--unit');
  if (idx < 0) return [1];
  const n = Number(process.argv[idx + 1]);
  if (!Number.isInteger(n) || n < 1 || n > LOP6_UNIT_COUNT) {
    throw new Error(`Invalid --unit (1–${LOP6_UNIT_COUNT})`);
  }
  return [n];
}
...
function contentPath(unit: number) {
  return resolve(process.cwd(), `scripts/data/lop6-unit${unit}-content.json`);
}
...
async function importUnit(unit: number, dryRun: boolean) {
  ...
  console.log(`\n=== Import GS6 ${lop6UnitCourseName(unit)} (${LOP6_LEVEL}) ===`);
  console.log(
  ...
  const course = await ensureLop6Course(prisma, unit);
  ...
  const speakingAction = await upsertSpeakingTopic(course.id, LOP6_LEVEL, raw.speakingTopic);
