...
import { gradeLookAndWriteExercise } from './gradeAnswer';

type LookAndWriteItem = {
  order: number;
  image: string;
  answer: string;
};

type LookAndWriteExercise = {
  id: string;
  index: number;
  title: string;
  instruction: string;
  word_bank: string[];
  items: LookAndWriteItem[];
};

type LookAndWriteGameResponse = {
  success: boolean;
  course?: {
    id: string;
    name: string;
  ...
  exercises?: LookAndWriteExercise[];
  unitTotal?: number;
...
function exercisePreview(exercise: LookAndWriteExercise): string {
  const sub = `${exercise.items.length} tranh · ${exercise.instruction || 'Look and write.'}`;
...
export function LookAndWriteGame({ courseId }: Props) {
  ...
  const [data, setData] = useState<LookAndWriteGameResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadGame() {
      ...
      try {
        const res = await fetch(`/api/games/look-and-write/${courseId}`, {
          signal: controller.signal,
        });
        const json = (await res.json()) as LookAndWriteGameResponse;
  ...
  const resetExerciseState = useCallback((exercise: LookAndWriteExercise | undefined) => {
    setPlacements({});
  ...
  async function handleCheck() {
    ...
    if (!allFilled) {
      setSubmitMessage(t('lookAndWrite.fillAll'));
      return;
    }
    ...
    try {
      const { isCorrect, itemResults } = gradeLookAndWriteExercise(
        placements,
  ...
  if (exercises.length === 0) {
    return (
      <div className="game-page law-page">
        <DataLoading variant="message" message={t('lookAndWrite.empty')} />
      </div>
    );
  }
    ...
    <div className="game-page law-page">
      ...
      {panel === 'list' ? (
        <div className="law-banner">
          <h2>{t('lookAndWrite.instruction')}</h2>
          <p>{course.name}</p>
        </div>
      ) : null}
      ...
      {panel === 'game' && currentExercise ? (
        <div className="game-card" id="gamePanel">
          ...
          <div className="law-worksheet" id="worksheet">
            ...
            >
              <span className="law-word-bank-label">{t('lookAndWrite.dragFromHere')}</span>
          ...
          {checkResult ? (
            <div className={`feedback show ${checkResult.isCorrect ? 'correct' : 'wrong'}`}>
              ...
              {checkResult.isCorrect
                ? t('lookAndWrite.allCorrect', { total: currentExercise.items.length })
                : t('lookAndWrite.partialCorrect', { correct: checkResult.correctCount, total: currentExercise.items.length })}
